...
pub fn part1(input: &str) -> usize {
    ...
    seeds
        .map(|seed| expanded_length(seed, 25, &mut cache))
        .sum()
}
...
fn expanded_length(
    ...
    let result = match iterate_one_number(number) {
        ...
        (num1, Some(num2)) => {
            expanded_length(num1, iterations - 1, cache)
                + expanded_length(num2, iterations - 1, cache)
        }
    };
...
pub fn part2(input: &str) -> usize {
    ...
    seeds
        .map(|seed| expanded_length(seed, 75, &mut cache))
        .sum()
}
